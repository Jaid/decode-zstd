...
export class Frame {
  ...
  private literals(reader: Reader, limit: number): Uint8Array {
    ...
    if (type < 2) {
      ...
      if (size > limit) invalid('Regenerated literals exceed the block or frame size.')
      if (type === 0) return reader.take(size)
    ...
    if (size > limit) invalid('Regenerated literals exceed the block or frame size.')
    if (format !== 0 && size < 6) invalid('Four-stream literals require at least six decoded bytes.')
    ...
    if (!this.huffman) invalid('Treeless literals have no previous Huffman table in this frame.')
  ...
  decode(reader: Reader, output: Output): void {
    ...
    if (descriptor & 8) invalid('Reserved frame header bit is set.')
    ...
    if (dictionary !== 0) throw new ZstdError('UNSUPPORTED_DICTIONARY', `Dictionary ${dictionary} is required; dictionaries are not supported.`)
    ...
    const blockMax = Math.min(window, 131072)
    let last = false
    while (!last) {
      ...
      if (size > blockMax && !(type === 2 && window === 0 && size === 2)) invalid('Block size exceeds the frame’s block maximum.')
      ...
      if (type < 2) {
        if (size > limit) invalid('Block exceeds the declared frame content size.')
        if (type === 0) output.append(reader.take(size))
        else output.repeat(reader.uint(1), size)
      } else {
    ...
    if (contentSize !== undefined && output.length - frameStart !== contentSize) invalid('Decoded frame content size does not match its header.')
    if (descriptor & 4) {
