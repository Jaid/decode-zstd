...
function readWeights(data: Uint8Array): number[] {
  ...
  const append = (state: number): void => {
    if (weights.length === 255) throw new Error('FSE decoded more than 255 Huffman weights.')
...
function buildHuffmanTable(weights: number[]): HuffmanTable {
  ...
  for (const weight of weights) {
    if (weight > 11) throw new Error(`Huffman weight ${weight} exceeds 11.`)
    if (weight !== 0) total += 2 ** (weight - 1)
  }
  if (total === 0) throw new Error('Huffman weights contain no present symbols.')
  const tableLog = Math.floor(Math.log2(total)) + 1
  if (tableLog > 11) throw new Error('Huffman tree depth exceeds 11.')
  ...
  for (const weight of weights) ranks[weight]++
  ...
  return {tableLog, symbols, bits}
}

/** Reads raw nibble or FSE-compressed Huffman weights, including the implicit last symbol. */
export function readHuffmanTable(data: Uint8Array, offset = 0): {table: HuffmanTable; next: number} {
  integer(offset, 0, data.length, 'Huffman byte offset')
  if (offset === data.length) throw new Error('Missing Huffman tree header.')
  ...
  if (header >= 128) {
    ...
    if (next > data.length) throw new Error('Truncated raw Huffman weights.')
    weights = []
    ...
    next = offset + header
    if (header === 0 || next > data.length) throw new Error('Invalid or truncated FSE Huffman weights.')
    weights = readWeights(data.subarray(offset, next))
  }
  return {table: buildHuffmanTable(weights), next}
}
...
function decodeStream(data: Uint8Array, output: Uint8Array, start: number, end: number, table: HuffmanTable): void {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (count < 1 || count > table.tableLog) throw new Error('Invalid Huffman table code length.')
    reader.read(count)
...
export function decodeHuffman(data: Uint8Array, outputSize: number, table: HuffmanTable, fourStreams: boolean): Uint8Array {
  ...
  if (outputSize > data.length * 8) throw new Error('Huffman output size exceeds available encoded bits.')
  if (!fourStreams) {
    const output = new Uint8Array(outputSize)
    decodeStream(data, output, 0, outputSize, table)
    return output
  }
  if (data.length < 10) throw new Error('Truncated Huffman jump table or four-stream payload.')
  const segmentSize = Math.ceil(outputSize / 4)
  if (segmentSize * 3 > outputSize) throw new Error('Huffman output size gives a negative fourth segment.')
  const ends = [6]
  for (let i = 0; i < 3; i++) {
    const length = data[i * 2] + data[i * 2 + 1] * 256
    if (length === 0) throw new Error('Huffman jump table contains an empty stream.')
    ends.push(ends[i] + length)
  }
  if (ends[3] >= data.length) throw new Error('Huffman jump table leaves no fourth stream or exceeds input.')
  ends.push(data.length)
  const output = new Uint8Array(outputSize)
  for (let i = 0; i < 4; i++) {
    decodeStream(data.subarray(ends[i], ends[i + 1]), output, i * segmentSize,
      i === 3 ? outputSize : (i + 1) * segmentSize, table)
  }
  return output
}
