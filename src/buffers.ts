...
export class Reader {
  ...
  take(size: number): Uint8Array {
    if (size > this.remaining) invalid('Truncated Zstandard input.')
  ...
  uint(size: number): number {
    ...
    let value = 0
    for (let i = size - 1; i >= 0; i--) value = value * 256 + bytes[i]
    if (!Number.isSafeInteger(value)) outputLimit('Frame content size exceeds JavaScript’s safe integer range.')
    return value
  }

  finish(): void {
    if (this.remaining !== 0) invalid('Unexpected trailing bytes in a compressed block.')
  }
}
...
export class Output {
  ...
  reserve(size: number): void {
    ...
    if (needed > this.limit) outputLimit(`Decoded data exceeds maxOutputSize (${this.limit} bytes).`)
    ...
    try {
      ...
      throw new ZstdError('OUTPUT_LIMIT', 'Unable to allocate the decoded output buffer.', {cause})
    }
  }
  ...
  finish(): Uint8Array {
    ...
    try {
      ...
      throw new ZstdError('OUTPUT_LIMIT', 'Unable to allocate the final output buffer.', {cause})
    }
  }
}
