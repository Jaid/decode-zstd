...
function compressedWeights(counts: number[], statesAndTransitions: readonly Field[], log = 5): Uint8Array {
  const data = Uint8Array.of(...normalized(counts, log).data, ...reverse(statesAndTransitions))
  return Uint8Array.of(data.length, ...data)
}

function encodeHuffman(symbols: readonly number[], table: HuffmanTable): Uint8Array {
  return reverse(symbols.map(symbol => {
    const index = table.symbols.indexOf(symbol)
    if (index < 0) throw new Error('Test symbol absent from table.')
    const bits = table.bits[index]
    return [index / 2 ** (table.tableLog - bits), bits]
  }))
}

function fourStreams(streams: Uint8Array[]): Uint8Array {
...
for (const [index, fixture] of reference.huffman.entries()) {
  test(`Huffman matches upstream v1.5.7 one/four-stream vectors: ${index}`, () => {
    ...
    for (let end = 0; end < header.length; end++) expect(() => readHuffmanTable(header.subarray(0, end))).toThrow()
...
test('Canonical codes match the specification; its ABEF byte example swaps E and F', () => {
  const weights = [...Array<number>(65).fill(0), 4, 3, 2, 0, 1]
  const {table} = readHuffmanTable(rawWeights(weights))
  expect(table.tableLog).toBe(4)
  expect([...table.symbols]).toEqual([69, 70, 67, 67, 66, 66, 66, 66, ...Array(8).fill(65)])
  expect([...table.bits]).toEqual([4, 4, 3, 3, 2, 2, 2, 2, ...Array(8).fill(1)])
...
test('Raw Huffman headers honor offsets, high nibbles and unused low-nibble padding', () => {
  ...
  const last128 = readHuffmanTable(rawWeights([...Array<number>(127).fill(0), 1])).table
...
test('FSE weights continue through zero-bit transitions and handle either terminal state', () => {
  ...
  for (let first = 0; first < 32; first++) {
    for (let second = 0; second < 32; second++) {
      ...
      if (zeroBits < 2) continue
      let expected: HuffmanTable
      try { expected = readHuffmanTable(rawWeights(weights)).table } catch { continue }
      const actual = readHuffmanTable(compressedWeights(counts, [[first, 5], [second, 5]])).table
      expect(actual).toEqual(expected)
      valid++
      if (weights.length % 2) odd++
      else even++
    }
  }
...
test('FSE weight streams enforce 255 explicit weights and log/symbol limits', () => {
  ...
  const table = buildFseTable(counts, 5)
  const one = table.symbols.indexOf(1)
  const fields: Field[] = [[one, 5], [one, 5], ...Array.from({length: 254}, (): Field => [0, 1])]
  ...
  const invalidSymbol = normalized([16, ...Array<number>(11).fill(0), 16], 5).data
...
test('Huffman supports maximum depth 11 and rejects invalid canonical trees', () => {
  ...
  for (const offset of [-1, 0.5, 3, NaN]) expect(() => readHuffmanTable(hex('8010'), offset)).toThrow(RangeError)
...
test('Four streams validate individual sizes, exact output partitions and every end marker', () => {
  ...
  for (const size of [1, 2, 5]) expect(() => decodeHuffman(data, size, table, true)).toThrow(/negative fourth/)
...
test('Huffman rejects invalid sizes and tables; zero output still validates its stream', () => {
  ...
  for (const data of [new Uint8Array(), hex('00'), hex('02')]) expect(() => decodeHuffman(data, 0, table, false)).toThrow()
  ...
  expect(() => decodeHuffman(hex('01'), 0, {...table, tableLog: 0}, false)).toThrow(RangeError)
  expect(() => decodeHuffman(hex('01'), 0, {...table, bits: new Uint8Array()}, false)).toThrow(/dimensions/)
  for (const bits of [Uint8Array.of(0, 1), Uint8Array.of(2, 1)]) {
    expect(() => decodeHuffman(hex('02'), 1, {...table, bits}, false)).toThrow(/code length/)
  }
})
