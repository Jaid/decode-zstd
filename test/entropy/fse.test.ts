...
test('FSE supports tiny, single-symbol and entirely low-probability synthetic tables', () => {
  for (let log = 0; log <= 16; log++) {
    ...
    const low = buildFseTable(new Array(size).fill(-1), log)
    ...
    for (let trial = 0; trial < 20; trial++) {
      ...
      for (const [symbol, count] of counts.entries()) {
        ...
        for (let state = 0; state < size; state++) {
          if (table.symbols[state] !== symbol) {
            continue
          }
          frequency++
          const end = table.base[state] + 2 ** table.bits[state]
          expect(end).toBeLessThanOrEqual(size)
          for (let next = table.base[state]; next < end; next++) {
            coverage[next]++
          }
        }
        expect(frequency).toBe(Math.abs(count))
...
test('FSE rejects invalid parameters, counts, symbol bounds, singleton wire tables and truncation', () => {
  ...
  for (const log of [-1, 17, 1.5, NaN, Infinity]) {
    expect(() => buildFseTable([1], log)).toThrow(RangeError)
  }
  const data = normalized([16, 16], 5).data
  for (const offset of [-1, 0.5, NaN, data.length + 1]) {
    expect(() => readFseTable(data, offset, 1, 5)).toThrow(RangeError)
  }
  for (const symbol of [-1, 256, 1.5, NaN]) {
    expect(() => readFseTable(data, 0, symbol, 5)).toThrow(RangeError)
  }
  for (const log of [4, 17, 5.5, NaN]) {
    expect(() => readFseTable(data, 0, 1, log)).toThrow(RangeError)
  }
