...
export const bytes = (...values: Array<number>): Uint8Array => Uint8Array.from(values)
...
export function frame(...blocks: Array<Uint8Array>): Uint8Array {
...
export function rleSequences(literalCode: number, offsetCode: number, matchCode: number, fields: Array<Field>, count = bytes(1)): Uint8Array {
...
export function sample(kind: string, length: number, seed = 0x12_34_56_78): Uint8Array {
  ...
  for (let i = 0; i < length; i++) {
    ...
    switch (kind) {
      ...
      case 'mixed': { result[i] = i % 16_000 < 4000 ? r & 255 : phrase[i % phrase.length]; break }
...
export {type Field, reverse} from './entropy/helpers.ts'
