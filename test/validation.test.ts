...
test('Every literal/match length code and its extra-bit range executes correctly', () => {
  expect(literalBase).toEqual([...Array.from({length: 16}, (_, i) => i), 16, 18, 20, 22, 24, 28, 32, 40, 48, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16_384, 32_768, 65_536])
  expect(matchBase).toEqual([...Array.from({length: 32}, (_, i) => i + 3), 35, 37, 39, 41, 43, 47, 51, 59, 67, 83, 99, 131, 259, 515, 1027, 2051, 4099, 8195, 16_387, 32_771, 65_539])
  for (const literal of [true, false]) {
    const bits = literal ? literalBits : matchBits
    const bases = literal ? literalBase : matchBase
    for (const [code, bit] of bits.entries()) {
      for (const extra of [0, 2 ** bit - 1]) {
        const length = bases[code] + extra
        const literalLength = literal ? length : 1
        const matchLength = literal ? 3 : length
        const sequence = rleSequences(literal ? code : 1, 0, literal ? 0 : code, [[extra, bit]])
        const data = frame(block(0, bytes(65, 65, 65, 65), false), compressed(rawLiterals(bytes(65), 3, literalLength), sequence))
        if (literalLength + matchLength > 131_072) {
          expect(() => decodeZstd(data)).toThrow(ZstdError)
        } else {
          expect(decodeZstd(data)).toEqual(new Uint8Array(4 + literalLength + matchLength).fill(65))
        }
      }
    }
  }
...
test('Unsigned offset fields, window mantissas and frame history isolation', () => {
  ...
  try {
    decodeZstd(concat(hex('28b52ffd230000008000'), block(0, bytes())))
  } catch (error) {
...
test('Deterministic malformed full-frame sweep stays within limits and uses typed errors', async () => {
  ...
  for (let trial = 0; trial < 5000; trial++) {
    ...
    try {
      output = decodeZstd(data, {maxOutputSize: 65_536})
    } catch (error) {
